...
class FacturaElectronicaController extends Controller
{
    ...
    public function generaXML($idOrden, $numdocumento, $claveacceso)
    {
        ...
        if ($res == 'OK') {
            ...
            if ($resSD) {
                ...
                $this->getpdfsriF($claveacceso, null);
            }
        }
    }
    ...
    public function getpdfsriF($nro_clave_acceso, $param = array())
    {
        ...
        //verifica si existe la ruta del xml en el storage

        $datosEmpresa = Empresa::where('id', 1)->first();

        if (Storage::exists('public/xml/' . $datosEmpresa->ruc . '/' . $nro_clave_acceso . '.xml')) {
            $xml = simplexml_load_file(storage_path('app/public/xml/' . $datosEmpresa->ruc . '/' . $nro_clave_acceso . '.xml'));
            ...
            $microempresa = isset($xml->infoTributaria->regimenMicroempresas) ? (string) $xml->infoTributaria->regimenMicroempresas : '';
            $contribuyenteRimpe = isset($xml->infoTributaria->contribuyenteRimpe) ? (string) $xml->infoTributaria->contribuyenteRimpe : '';
            $agenteretencion = isset($xml->infoTributaria->agenteRetencion) ? (string) $xml->infoTributaria->agenteRetencion : '';
            ...
            exit('No existe el XML. getpdfsriF');
        }


        //Consultar logo
        $path = asset('logo/logo_header.jpeg');
        ...
        // Crear una imagen a partir del código de barras
        $image = Image::make($barcode);

        // Añadir el número debajo del código de barras
        $image->text($number, 10, 60, function ($font) {
            $font->size(20);
            $font->color('#000000');
        });

        // Guardar la imagen en el sistema de archivos
        $barcodePath = 'barcodes/' . $number . '.png';
        Storage::disk('public')->put($barcodePath, (string) $image->encode());

        $pathBarcode = asset('storage/barcodes/' . $clave_acceso . '.png');
        ...
        $html .= '            <img alt="SIN LOGO" class="imgLogo" src="' . $path . '" /><br><br><br><br>';
        $html .= '        </div>';
        ...
        // if($ruc=="1891758584001"){
        //     $html.='        &nbsp;&nbsp;&nbsp;&nbsp;<label><strong>Agente de Retención</strong> Resolución N° 1</label><br><br>';
        //     $html.='        &nbsp;&nbsp;&nbsp;&nbsp;<label><strong>Contribuyente Régimen Microempresa</strong></label><br><br>';
        // }
        ...
        if ($microempresa != '') {
            $html .= '            <label> ' . $microempresa . '</label><br><br>';
        }
        if ($contribuyenteRimpe != '') {
            $html .= '            <label> ' . $contribuyenteRimpe . '</label><br><br>';
        }
        if ($agenteretencion != '') {
            $html .= '            <label>AGENTE DE RETENCIÓN RESOLUCIÓN N° ' . $agenteretencion . '</label><br>';
        }
        $html .= '        </div>';
        $html .= '    </div>';
        ...
        $html .= '        <b><label>CLAVE DE ACCESO:</label></b><br><br>';
        $html .= '        <img alt="ERROR" class="imgBarcode" src="' . $pathBarcode . '" /><br><br><br><br>';
        $html .= '    </div>';
        $html .= '</div>';
        ...
        if ($ruc === '1891745466001') {
            $html .= '            <td>Detalle Adicional</td>';
        }
        ...
        foreach ($detalles as $dtprod) {
            ...
            if ($ruc === '1891745466001') {
                $html .= '            <td>' . (string) $dtprod->unidadMedida . '</td>';
            }
        ...
        foreach ($infoAdicional->campoAdicional as $value) {
            /*if(strlen($microempresa)>0) {
                if ((string)$value['nombre'] !== 'REGIMEN') {
                    $html.='                <tr>';
        ...
        $html .= '            <tr>';
        $html .= '                <td>ICE</td>';
        $html .= '                <td class="txtDerecha">' . number_format((float)$icetotal, 2, '.', '') . '</td>';
        $html .= '            </tr>';
        $html .= '            <tr>';
        $html .= '                <td>IVA 12%</td>';
        ...
        $output = $pdf->output();
        $directory = 'public/xml/' . $datosEmpresa->ruc;
        $filePath = $directory . '/' . $clave_acceso . '.pdf';

        // Asegurarse de que el directorio exista
        Storage::makeDirectory($directory);

        // Guardar el PDF en el sistema de archivos de Laravel
        Storage::put($filePath, $output);

        // Obtener la ruta absoluta al archivo, si es necesario
        $rutaAbsoluta = Storage::path($filePath);


        $datosFactura = FacturaElectronica::with('orden')->where('clave_acceso', $clave_acceso)->first();
        $datosCliente = DatosFacturacion::with('cliente')->where('id', $datosFactura->orden->datosfacturacion_id)->first();
        $usuario = User::where('id', $datosCliente->cliente->usuario_id)->first();

        $data = new \stdClass();

        $data->cliente = $datosCliente->nombre;
        $data->documento = $datosFactura->ruc_cedula;
        $data->factura = $datosFactura->factura;
        $data->clave_acceso = $datosFactura->clave_acceso;
        $data->pdf = $rutaAbsoluta;

        //envia el correo con la clave
        try {
            //adjunta el pdf y pasarle la data  a EnviarFactura
            $correo =  Mail::to($usuario->email)->send(new EnviarFactura($data));
        } catch (\Exception $e) {
